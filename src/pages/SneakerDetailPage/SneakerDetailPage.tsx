...
import SneakerDetailPageStyled from "./SneakerDetailPageStyled";

const SneakerDetailPage = (): React.ReactElement => {
  const { id } = useParams();
  const { deleteSneaker, getSneakerById } = useApi();
  const { id: userId, isLogged } = useAppSelector((state) => state.userStore);
  const sneaker = useAppSelector((state) => state.sneakersStore.sneaker);
    ...
    (async () => {
      if (isLogged) {
        if (id) {
          ...
          if (sneaker) {
            dispatch(loadSneakerByIdActionCreator(sneaker));
          }
        }
      }
    })();
  }, [dispatch, getSneakerById, isLogged, id]);
  ...
  const {
    ...
    price,
    colors,
    features: { description, description2, isAvailable },
  } = sneaker;

  return (
    <SneakerDetailPageStyled>
      ...
      <div className="info-container">
        ...
        <div className="info">
          <span className="info__title">Colors:</span>
          <span className="info__value">{colors.join(", ")}</span>
        </div>
        <div className="info">
          <span className="info__title">Description:</span>
          <ul className="info__value">
            <li className="info__value__list">{description}</li>
            {description2 && (
              <li className="info__value__list">{description2}</li>
            )}
          </ul>
        </div>
      </div>
      {isAvailable && <h3 className="availability">Commercially available!</h3>}
      {!isAvailable && <h3 className="availability">Model discontinued ...</h3>}
    </SneakerDetailPageStyled>
  );
};
